...
class elph_structure_single_q():
 def __init__(self,phh_structure,q):
  ...
  self.lambda_or_a2f='' #'lambda' or 'elph'
  self.COLORS=[]
  self.q_no=q

 def make_kpoints_single_q(self,basic_structure,q,q_cryst,qstar,structure_new=0):
  ...
  #self.KPOINTS_all=structure_new.allk
   ...
   self.KPOINTS_all_cryst_orig=copy.deepcopy(self.KPOINTS_all_cryst)
   self.KPOINTS_cryst_orig=copy.deepcopy(self.KPOINTS_cryst)
  ...
  self.nkp=len(self.KPOINTS_cryst)
...
#  self.KPOINTS_all_all_q.append(structure_new.allk)
  ...
  for k in  self.KPOINTS_cryst:
   self.KPQ.append(structure_new.find_k_plus_q(k, self.KPOINTS_all_cryst_round2,q_cryst))
  # print(k,self.KPQ[-1])
#  print(self.KPQ)
  ...
  multik=np.zeros((len(basic_structure.NONEQ_cryst)))
 ...
 def make_kpoints_dense_single_q(self,basic_structure_dense,q,q_cryst,qstar):
  structure_new=copy.deepcopy(basic_structure_dense)
  structure_new.SYMM_crystal=self.SYMM_q
  structure_new.find_noneq()
  self.make_kpoints_single_q(basic_structure_dense,q,q_cryst,qstar,structure_new)
...
#       elph_k_ii=symmetrize((elph_k_ii))
       for numiband,iband in enumerate(ell_structure.bands_num):
        for numjband,jband in enumerate(ell_structure.bands_num):
         ELPH[numiband][numjband][k-1][nmode]=elph_k[iipert*self.nbnd_el*self.nbnd_el+iband*self.nbnd_el+jband]
 #        ELPH[numiband][numjband][k-1][nmode]+=elph_k[iband*self.nbnd_el*npert+jband*npert+iipert] #--wrong
...
#       elph_k_ii=symmetrize((elph_k_ii))
 ...
 def interpolate_elph_single_q(self,phh_structure,ell_structure,structure): 
  ELPH_wholegrid=np.array([ self.ELPH[:,:,m[3],:] for m in self.KPOINTS_all_cryst_orig]).transpose((1,2,0,3))
  which_noneq_in_allk=[]
  for ni,i in enumerate(self.KPOINTS_cryst_orig):
   for nj,j in enumerate(self.KPOINTS_all_cryst):
    if j[3]==ni: 
     which_noneq_in_allk.append(nj)
     break
#  print(ELPH_wholegrid.shape)
 # np.zeros(shape=(self.fermi_nbnd_el,self.fermi_nbnd_el,\
 #               len(structure.allk),phh_structure.no_of_modes), dtype=complex) #stores elph[k][ibnd][jbnd][nmode]
  ELPH=[]
  for ib in ELPH_wholegrid:
    ELPH.append([])
    for nm in range(phh_structure.no_of_modes):
     interp_elph=(structure.interpolate_data(ib[:,:,nm]))
     ELPH[-1].append([interp_elph[:,nk] for nk in which_noneq_in_allk])
  ELPH=np.array(ELPH).transpose((0,2,3,1))
  print('shape',ELPH.shape)
  return ELPH
 ...
 def sum_elph_over_jband(self,phh_structure,ell_structure,structure): 
  ...
  for iband in range(self.fermi_nbnd_el):
    for numk, k in enumerate(self.KPOINTS_cryst): 
    # if abs(ell_structure.ENE_fs[iband][k[4]])>2: continue
     for jband in range(self.fermi_nbnd_el):
     # if abs(ell_structure.ENE_fs[jband][self.KPQ[numk][2]])>2: continue
...
ELPH2 , structure.at,structure.e, structure.SYMM_crystal, self.SYMM_q,self.irt,self.rtau,phh_structure.Q_crystal[self.q_no-1] )
 ...
 def calc_lambda_summed_over_modes(self,structure,phh_structure,ell_structure,l_or_gep):
  ...
  wagi0=[[el_structure.w0gauss(-ell_structure.ENE_fs[jband][k[4]]) for numk,k in enumerate(self.KPOINTS_cryst)] for jband in range(self.fermi_nbnd_el) ]
  wagi=[[wagi0[jband][numk]*self.WEIGHTS_OF_K[numk] for numk in range(len(self.KPOINTS_cryst))] for jband in range(self.fermi_nbnd_el) ]
  sumwag=np.sum(wagi)
  if ell_structure.soc!=0: factor=1 #sumwag=np.sum(wagi)/2 #devide by 2 because we are suming not averaging over spin
  else: factor=2
  if self.lambda_or_a2f=='a2f':
   self.COLORS=np.zeros(shape=(self.fermi_nbnd_el, 500),dtype=complex)
   Wall=np.linspace(0,np.max(phh_structure.FREQ),500)
   for w in Wall:
    for num in range(phh_structure.no_of_modes): #modes
     ...
     for numk in range(self.nkp):
      for jband in range(self.fermi_nbnd_el): 
        lam= factor*(self.ELPH_sum[jband][numk][num]) /((phh_structure.FREQ[self.q_no-1][num]))*wagi[jband][numk] *    el_structure.w0gauss(w-phh_structure.FREQ[self.q_no-1][num],0.002)
   ...
   for num in range(phh_structure.no_of_modes): #modes
    ...
    for numk in range(self.nkp):
     for jband in range(self.fermi_nbnd_el): 
       lam= factor*(self.ELPH_sum[jband][numk][num]) /((phh_structure.FREQ[self.q_no-1][num])**2)
       self.COLORS[jband][numk]+=lam  
  ...
  print(self.q_no,self.COLORS2,'    ',np.round(sum(self.COLORS2),4),'from main program')  
#  lambda_sum0=sum([sum([ self.COLORS[jband][numk]*wagi[jband][numk] for numk in range(len(self.COLORS[jband]))]) for jband in range(len(self.COLORS)) ])
 ...
 def elph_single_q_in_whole_kgrid(self,structure,phh_structure,ell_structure,l_or_gep):
  wagi0=[[el_structure.w0gauss(-ell_structure.ENE_fs[jband][k[4]]) for numk,k in enumerate(self.KPOINTS_cryst)] for jband in range(self.fermi_nbnd_el) ]
  wagi=np.array([[ bnd[k[3]] for k in self.KPOINTS_all_cryst] for bnd in wagi0])
  self.COLORS=np.array([[ bnd[k[3]] for k in self.KPOINTS_all_cryst] for bnd in self.COLORS])
   ...
   for nj,j in enumerate(tmp):
    ...
    if m==4:
     for k in tmp[nj+2:]:
      ...
      try: self.lambdas_from_file.append(np.round(float(k.split()[2]),4))
      except: self.lambdas_from_file.append(0)
      if self.lambdas_from_file[-1]<0: self.lambdas_from_file[-1]=0
     break
  ...
  #  ELPH2=np.zeros(shape=(len(structure.NONEQ),phh_structure.no_of_modes,phh_structure.no_of_modes), dtype=complex) #stores elph[k][ibnd][jbnd][nmode]
    for numk, k in enumerate(self.KPOINTS_cryst): 
     ...
     for jband in range(self.fermi_nbnd_el):
      ...
      for iipert in range(phh_structure.no_of_modes):
       for jjpert in range(phh_structure.no_of_modes):
        ELPH2[iipert][jjpert]+=np.conjugate(self.ELPH[iband][jband][numk][iipert])*self.ELPH[iband][jband][numk][jjpert] *weight 
  ELPH2 =ph_structure.symmetrize(phh_structure.nat,np.array(phh_structure.PATT[self.q_no-1]),
ELPH2 , structure.at,structure.e, structure.SYMM_crystal, self.SYMM_q,self.irt,self.rtau,phh_structure.Q_crystal[self.q_no-1] )
  lam=[]
  for nu in range(phh_structure.no_of_modes):
...
#ELPH2[numk] , structure.at,structure.e, structure.SYMM_crystal, structure.SYMM_crystal,structure.irt,self.rtau,phh_structure.Q_crystal[q_point_no-1] )
  if ell_structure.soc: lam=2*np.array(lam)
  print('\n',self.q_no,(np.round([lam[m] for m in phh_structure.ORDER_OF_IRR[self.q_no-1]] ,4)),'    ',np.round(sum(lam),4),'calc as elphon')
  self.read_lambdas_from_file()
  print(self.q_no,self.lambdas_from_file,'    ',np.round(sum(self.lambdas_from_file),4),'from elph_dir')
